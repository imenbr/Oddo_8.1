    ...
    def generate_records(self, cr, uid, ids, data, context):
        ...
        for order_id in order_ids :
            ...
            for order_line in order_lines:
                ...
                #Rim modif 26/06/2015 qte liv & %
                cr.execute('''
                SELECT
                sum(m.product_qty) as qte_livree
                FROM
                stock_move m
                LEFT JOIN
                purchase_order_line p on (p.id=m.purchase_line_id)
                WHERE
                p.order_id = %s AND p.product_id =%s AND m.state = 'done' ''',(order_id,order_line['product_id']))
                #Cela permet de lister tous les résultats de la table stock_move même s’il n’y a pas de correspondance dans la deuxième tables purchase_order_line.
                res_qte_livree = cr.dictfetchone()['qte_livree']
                if res_qte_livree:
                    qte_livree = res_qte_livree
                else:
                    qte_livree = 0
                #date = move['date']
