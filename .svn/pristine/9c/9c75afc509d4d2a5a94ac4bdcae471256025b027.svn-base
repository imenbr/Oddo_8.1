...
class jasper_client(JasperDataParser.JasperDataParser):
    ...
    def generate_records(self, cr, uid, ids, data, context):
        ...
        if 'form' in data: 
            ...
            cr.execute("SELECT * FROM reglement_paiement where partner_id = %s and date_reglement <= %s and state = 'close' and type='out' ",  (fournisseur_id,datedebut,))
            ...
            cr.execute("SELECT * FROM reglement_retenu_source where partner_id = %s and date_retenu <= %s and state = 'close' and type='out' and recu='True'",  (fournisseur_id,datedebut,))
            ...
            cr.execute("select * FROM account_invoice where type='out_invoice' and partner_id = %s and date_invoice <= %s  ",  (fournisseur_id,datedebut,))
            ...
            cr.execute("select * FROM account_invoice where type='out_refund' and partner_id = %s and date_invoice <= %s  ",  (fournisseur_id,datedebut,))
            invoices = cr.dictfetchall()
            for invoice in invoices:  
                #ReportDebit += invoice['amount_total'] 
                ReportCredit += invoice['amount_total'] 
            #**************************************** PIECE PAIEMENT precedant :: CHQ TRT ************************************
            cr.execute("select * FROM reglement_piece,reglement_mode where reglement_piece.mode_reglement = reglement_mode.id and reglement_piece.partner_id = %s and reglement_piece.type='out' and reglement_piece.date_echance <= %s  and reglement_piece.state='cashed' and (reglement_mode.code ='CHQ' or  reglement_mode.code ='TRT') ",  (fournisseur_id,datedebut,))
            ...
            cr.execute("select * FROM reglement_piece,reglement_mode where reglement_piece.mode_reglement = reglement_mode.id and reglement_piece.partner_id = %s and reglement_piece.type='out' and reglement_piece.date_echance <= %s and (reglement_mode.code ='ESP' or  reglement_mode.code ='VIR')",  (fournisseur_id,datedebut))
            pieces = cr.dictfetchall()
            for piece in pieces:  
                #ReportDebit += piece['montant_piece']
                ReportCredit += piece['montant_piece']
            ...
            #**************************************** Facture Fournisseur  *********************************
            cr.execute("select * FROM account_invoice where type='out_invoice' and partner_id = %s and date_invoice >= %s and date_invoice <= %s  ",  (fournisseur_id,datedebut,datefin,))
            invoices = cr.dictfetchall()
            for invoice in invoices:   
            ...
            #**************************************** montant en espece *********************************
            cr.execute("SELECT * FROM reglement_paiement where partner_id = %s and date_reglement <= %s and date_reglement >= %s and type='out' and state = 'close'",  (fournisseur_id,datefin,datedebut,))
            reglements = cr.dictfetchall()
            for reglement in reglements:
            ...
            cr.execute("select * FROM account_invoice where type='out_refund' and partner_id = %s and date_invoice >= %s and date_invoice <= %s  ",  (fournisseur_id,datedebut,datefin,))
            invoices = cr.dictfetchall()
            for invoice in invoices:  
            ...
            cr.execute("select reglement_piece.id as p_id,montant_piece,date_echance,mode_reglement,num_cheque_traite,montant_piece FROM reglement_piece,reglement_mode where reglement_piece.mode_reglement = reglement_mode.id and reglement_piece.partner_id = %s and reglement_piece.type='out' and reglement_piece.date_echance >= %s and reglement_piece.date_echance <= %s and reglement_piece.state='cashed' and (reglement_mode.code ='CHQ' or  reglement_mode.code ='TRT')",  (fournisseur_id,datedebut,datefin,))
            pieces = cr.dictfetchall()
            for piece in pieces:   
                #rim modif :: sttc :: 18/07/2014 ::
                invoices=''
                ...
                date_piece = piece['date_echance'].split("-")[2]+"/"+piece['date_echance'].split("-")[1]+"/"+piece['date_echance'].split("-")[0]
            ...
            cr.execute("select reglement_piece.id as p_id,mode_reglement,montant_piece,num_cheque_traite,reglement_piece.date_echance as date_piece  FROM reglement_piece,reglement_mode where reglement_piece.mode_reglement = reglement_mode.id and reglement_piece.partner_id = %s and reglement_piece.type='out' and reglement_piece.date_echance >= %s and reglement_piece.date_echance <= %s and (reglement_mode.code ='ESP' or  reglement_mode.code ='VIR')",  (fournisseur_id,date_obj_debut,date_obj_fin,))
            pieces = cr.dictfetchall()
            for piece in pieces:   
                #rim modif :: sttc :: 18/07/2014 ::
                invoices=''
        ...
        #**************************************** AVANCE ************************************
            ...
            cr.execute("select *  FROM reglement_avance where reglement_avance.partner_id = %s and reglement_avance.type='client' and reglement_avance.date_avance >= %s and reglement_avance.date_avance <= %s ",  (fournisseur_id,datedebut,datefin,))
            pieces = cr.dictfetchall()
            for piece in pieces:   
                #rim modif :: sttc :: 18/07/2014 ::
                invoices=''
