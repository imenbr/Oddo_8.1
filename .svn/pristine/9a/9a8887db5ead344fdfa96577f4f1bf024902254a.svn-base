...
class invoice_picking(osv.osv):
	...
	_columns = {
		'internal_number': fields.char('Numero', size=32,help="Unique number of the invoice"),
		'number': fields.char('Code Facture', size=32,help="Unique number of the invoice"),
		'date_invoice_picking': fields.date('Date Facture', required=True,),
		'partner_id': fields.many2one('res.partner', 'partner_id', ),
		#'invoice_picking_line':fields.one2many('invoice.picking.line', 'invoice_picking_id', 'Invoice Picking Line', readonly=True, states={'draft':[('readonly',False)]}),
                'invoice_picking_line':fields.one2many('invoice.picking.line', 'invoice_picking_id', 'Invoice Picking Line',),
		...
		'state': fields.selection([('draft','Brouillon'),('open','Ouverte')],'State', select=True, ),
...
class invoice_picking_line(osv.osv):
	...
	'date_picking': fields.related('picking_id','date', type='date', readonly=False, relation='stock.picking', store=True, string='Date'),
