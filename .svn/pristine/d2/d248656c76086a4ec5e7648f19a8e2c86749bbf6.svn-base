...
class purchase_order(osv.osv):
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            if order.partner_id.exoner==False:
	...
	'order_line': fields.one2many('purchase.order.line', 'order_id', 'Order Lines',states={'approved':[('readonly',False)],
                                              'done':[('readonly',False)]},
                                      copy=True),
	'pricelist_id':fields.many2one('product.pricelist', 'Pricelist', states={'confirmed':[('readonly',True)], 'approved':[('readonly',True)],'done':[('readonly',True)]}, help="The pricelist sets the currency used for this purchase order. It also computes the supplier price for the selected products/quantities."),
        'date_shipping':fields.date('Date de Livraison',states={'confirmed':[('readonly',False)], 'approved':[('readonly',False)]}, help="Date de livraison"),
	...
	'date_order':fields.datetime('Order Date', required=True, states={'confirmed':[('readonly',False)],
