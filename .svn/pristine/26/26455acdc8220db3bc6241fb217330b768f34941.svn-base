		...
		if var<0:
		  raise osv.except_osv(_('Les montants ont ete modifies !'),_("Veuillez verifier la facture %s" %rd.invoice_id.number,))
                if(round(rd.montant,3)<round(difference,3)):
                  cr.execute('UPDATE  account_invoice SET montant_paye= montant_paye + %s , state=%s , reste_a_payer=%s WHERE id=%s', (rd.montant, 'ppaid',rd.montant_rest-rd.montant , rd.invoice_id.id,))
                else:
                  cr.execute('UPDATE  account_invoice SET montant_paye= montant_paye + %s , state=%s, reste_a_payer=%s WHERE id=%s', (rd.montant, 'paid', rd.montant_rest-rd.montant, rd.invoice_id.id,))
		...
		if var<0:
		  raise osv.except_osv(_('Les montants ont ete modifies !'),_("Veuillez verifier avoir numero %s" %rda.avoir_id.number,))
                if(rda.montant<(ttc-montant_paye)):
                  ...
                  cr.execute('UPDATE  account_invoice SET montant_paye= montant_paye + %s , state=%s, reste_a_payer=%s WHERE id=%s', (rda.montant, 'ppaid',rda.montant_rest-rda.montant, rda.avoir_id.id,))
                else:
                  cr.execute('UPDATE  account_invoice SET montant_paye= montant_paye + %s , state=%s, reste_a_payer=%s WHERE id=%s', (rda.montant, 'paid',rda.montant_rest-rda.montant, rda.avoir_id.id,))
