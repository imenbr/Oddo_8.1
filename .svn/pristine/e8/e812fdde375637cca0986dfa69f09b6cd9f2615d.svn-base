...
class sale_devis(osv.osv):
    ...
    def action_view_order(self, cr, uid, ids, context=None):
     for devis in self.browse(cr, uid, ids):
        ...
        cr.execute("select id from sale_order where devis_id = %s",(devis.id,))
        res = cr.dictfetchone()
    ...
    _columns = {
        ...
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
        'amount_untaxed':fields.float(string='Total HT', digits_compute=dp.get_precision('Account'),
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
        'amount_tax':fields.float(string='Total Taxe', digits_compute=dp.get_precision('Account'),
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
        'amount_total':fields.float(string='Total', digits_compute=dp.get_precision('Account'),
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
...
class sale_devis_line(osv.osv):
    ...
    def _amount_line(self):
        ...
        '''
        self.price_unit_discount = self.price_unit * (1 - (self.discount or 0.0) / 100.0)
    ...
    @api.depends('price_unit', 'discount', 'tax_ids', 'product_qty',
        'product_id', 'devis_id.partner_id')#, 'devis_id.currency_id')
    def _amount_line_price(self):
        '''
           Methode qui permet de calculer le prix unitaire apres remise d'une ligne de devis
        '''
    ...
    _columns = {
        ...
        'designation': fields.char('Désignation (commentaire)'),
        'price_unit': fields.float('Prix', digits_compute= dp.get_precision('Account')),
	'price_unit_discount': fields.float('Prix unitaire apres remise', digits_compute= dp.get_precision('Account'),store=True, readonly=True,compute='_amount_line'),
        'product_qty': fields.float('Quantité',digits_compute= dp.get_precision('Discount')),
        'discount': fields.float('Remise'),
        'product_uom': fields.many2one('product.uom', 'Unité de mesure', required=False),
        'tax_ids': fields.many2many('account.tax', 'sale_devis_line_tax', 'devis_line_id', 'tax_id', 'Taxes', ),
        'price_subtotal':fields.float(string='Sous Total', digits_compute= dp.get_precision('Account'),
	...
	res_final = {}
        if product:
            ...
            #print "nameeeeeeeeeee designation : =====",product_obj.name_template
	    ...
	    taxes = self.pool.get('product.product').browse(cr,uid,product,context=context)
            ...
            res_final = {'value':{'price_unit':price,'designation':designation ,'tax_ids': taxes.taxes_id}}
        return res_final
...
class sale_devis_taxe(osv.osv):
    ...
    _columns = {
        ...
        'name': fields.char('Taxe', size=64, required=True),
        'account_id': fields.many2one('account.account', 'Compte', required=True, domain=[('type','<>','view'),('type','<>','income'), ('type', '<>', 'closed')]),
        'base': fields.float('Base', digits_compute=dp.get_precision('Account')),
        'amount': fields.float('Montant', digits_compute=dp.get_precision('Account')),
        'manual': fields.boolean('Manual'),
        'sequence': fields.integer('Séquence', help="Gives the sequence order when displaying a list of invoice tax."),
        'base_code_id': fields.many2one('account.tax.code', 'Code Base', help="The account basis of the tax declaration."),
        'base_amount': fields.float('Base Code Amount', digits_compute=dp.get_precision('Account')),
        'tax_code_id': fields.many2one('account.tax.code', 'Code Taxe', help="The tax basis of the tax declaration."),
        'tax_amount': fields.float('Tax Code Amount', digits_compute=dp.get_precision('Account')),
        'company_id': fields.related('account_id', 'company_id', type='many2one', relation='res.company', string='Société', store=True, readonly=True),
