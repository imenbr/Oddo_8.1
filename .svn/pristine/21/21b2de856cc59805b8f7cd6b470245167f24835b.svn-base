...
class jasper_client(JasperDataParser.JasperDataParser):
    ...
    def generate_records(self, cr, uid, ids, data, context):
        ...
        if 'form' in data: 
            ...
            cr.execute("SELECT * FROM reglement_paiement where partner_id = %s and date_reglement <= %s and state = 'close' and type='in' ",  (fournisseur_id[0],datedebut,))
            ...
            cr.execute("SELECT * FROM reglement_retenu_source where partner_id = %s and date_retenu <= %s and state = 'close' ",  (fournisseur_id[0],datedebut,))
            retenues = cr.dictfetchall()
            for retenue in retenues: 
                ReportCredit += retenue['total_retenu']
            #**************************************** Facture Fournisseur precedant *********************************
            cr.execute("select * FROM account_invoice where  state!='draft' and state!='cancel' and type='in_invoice' and partner_id = %s and date_invoice <= %s  ",  (fournisseur_id[0],datedebut,))
            invoices = cr.dictfetchall()
            for invoice in invoices:   
                ReportDebit += invoice['amount_total']
            #**************************************** Facture Avoir Fournisseur precedant *********************************
            cr.execute("select * FROM account_invoice where  state!='draft' and state!='cancel' and  type='in_refund' and partner_id = %s and date_invoice <= %s  ",  (fournisseur_id[0],datedebut,))
            invoices = cr.dictfetchall()
            for invoice in invoices:  
                ReportDebit += invoice['amount_total'] * (-1) 
            #**************************************** PIECE PAIEMENT precedant ************************************
            cr.execute("select * FROM reglement_piece where partner_id = %s and create_date <= %s  ",  (fournisseur_id[0],datedebut,))
            ...
            #**************************************** montant en espece *********************************
            cr.execute("SELECT * FROM reglement_paiement where partner_id = %s and date_reglement <= %s and date_reglement >= %s and state = 'close' and type='in' ",  (fournisseur_id[0],datefin,datedebut,))
            reglements = cr.dictfetchall()
            for reglement in reglements:
            ...
            #**************************************** Retenue a la source *********************************
            cr.execute("SELECT * FROM reglement_retenu_source where partner_id = %s and date_retenu <= %s and date_retenu >= %s and state = 'close' ",  (fournisseur_id[0],datefin,datedebut,))
            retenues = cr.dictfetchall()
            for retenue in retenues: 
                ...
                result.append(data) 
            #**************************************** Facture Fournisseur  *********************************
            cr.execute("select * FROM account_invoice where  state!='draft' and state!='cancel' and type='in_invoice' and partner_id = %s and date_invoice >= %s and date_invoice <= %s  ",  (fournisseur_id[0],datedebut,datefin,))
            invoices = cr.dictfetchall()
            for invoice in invoices:   
            ...
            cr.execute("select * FROM account_invoice where  state!='draft' and state!='cancel' and  type='in_refund' and partner_id = %s and date_invoice >= %s and date_invoice <= %s  ",  (fournisseur_id[0],datedebut,datefin,))
            invoices = cr.dictfetchall()
            for invoice in invoices:  
                ...
                result.append(data) 
            #**************************************** PIECE PAIEMENT ************************************
            cr.execute("select * FROM reglement_piece where partner_id = %s and create_date >= %s and create_date <= %s  ",  (fournisseur_id[0],datedebut,datefin,))
            pieces = cr.dictfetchall()
            for piece in pieces:   
                type_piece = ""
